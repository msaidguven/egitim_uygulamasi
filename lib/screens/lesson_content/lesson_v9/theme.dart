...
class FontSizeNotifier extends ChangeNotifier {
  ...
  void increase() { if (_scale < 1.5) { _scale = (_scale + 0.15).clamp(0.7, 1.5); notifyListeners(); } }
  void decrease() { if (_scale > 0.7) { _scale = (_scale - 0.15).clamp(0.7, 1.5); notifyListeners(); } }
...
class TC {
  ...
  Color get surface => isDark ? const Color(0xFF22223B) : const Color(0xFFFFFFFF);
  Color get card    => isDark ? const Color(0xFF2D2D4E) : const Color(0xFFFFFFFF);
  Color get border  => isDark ? const Color(0xFF3D3D6B) : const Color(0xFFCDD5F0);
  Color get muted   => isDark ? const Color(0xFF6B6B9A) : const Color(0xFF9090BB);
  Color get subtle  => isDark ? const Color(0xFF9090BB) : const Color(0xFF6B6B9A);

  // ── Metin ────────────────────────────────────────────────────────────────
  Color get textBody  => isDark ? const Color(0xFFF0F0FF) : const Color(0xFF1A1A2E);
  Color get textMuted => isDark ? const Color(0xFFAAAAAA) : const Color(0xFF5A5A80);

  // ── İçerik kutuları ───────────────────────────────────────────────────────
  // Dark: koyu pastel  /  Light: parlak cıvıl renkler
  Color get cBg        => isDark ? const Color(0xFF252545) : const Color(0xFFFFFFFF);
  Color get cBgBlue    => isDark ? const Color(0xFF0D1E30) : const Color(0xFFD6EEFF);
  Color get cBgGreen   => isDark ? const Color(0xFF0A2012) : const Color(0xFFCCF5DD);
  Color get cBgOrange  => isDark ? const Color(0xFF2A1400) : const Color(0xFFFFE8CC);
  Color get cBgPurple  => isDark ? const Color(0xFF1C0A35) : const Color(0xFFEBDEFF);
  Color get cBgTeal    => isDark ? const Color(0xFF002825) : const Color(0xFFCCF2EE);
  Color get cBgYellow  => isDark ? const Color(0xFF221800) : const Color(0xFFFFF4CC);
  Color get cBgPink    => isDark ? const Color(0xFF280010) : const Color(0xFFFFD6E8);

  // ── İçerik kutusu metin ───────────────────────────────────────────────────
  Color get cText     => isDark ? const Color(0xFFF0F0FF) : const Color(0xFF1A1A2E);
  Color get cTextMid  => isDark ? const Color(0xFFCCCCEE) : const Color(0xFF2D3561);
  Color get cTextSoft => isDark ? const Color(0xFF9090BB) : const Color(0xFF4A5080);

  // ── Durum ─────────────────────────────────────────────────────────────────
  Color get green    => isDark ? const Color(0xFF4CAF50) : const Color(0xFF2E7D32);
  Color get greenLt  => isDark ? const Color(0xFF0D200D) : const Color(0xFFCCF5DD);
...
class AppTheme {
  ...
  static const LinearGradient primaryGrad = LinearGradient(colors: [Color(0xFF5B4FE8), Color(0xFF9C6FFF)], begin: Alignment.topLeft, end: Alignment.bottomRight);
  static const LinearGradient certGrad    = LinearGradient(colors: [Color(0xFFF9A825), Color(0xFFFF6F00)], begin: Alignment.topLeft, end: Alignment.bottomRight);
  static const LinearGradient introGrad   = LinearGradient(colors: [Color(0xFF0288D1), Color(0xFF01579B)], begin: Alignment.topLeft, end: Alignment.bottomRight);
  static const LinearGradient gameGrad    = LinearGradient(colors: [Color(0xFF558B2F), Color(0xFF00897B)], begin: Alignment.topLeft, end: Alignment.bottomRight);
  static const LinearGradient wordGrad    = LinearGradient(colors: [Color(0xFFC2185B), Color(0xFF6A1B9A)], begin: Alignment.topLeft, end: Alignment.bottomRight);
  ...
  static ThemeData _build(bool dark) => ThemeData(
    ...
    textTheme: TextTheme(
      headlineLarge:  TextStyle(color: dark ? textBody  : const Color(0xFF1A1A2E), fontWeight: FontWeight.w800, fontSize: 22),
      headlineMedium: TextStyle(color: dark ? textBody  : const Color(0xFF1A1A2E), fontWeight: FontWeight.w800, fontSize: 18),
      bodyMedium:     TextStyle(color: dark ? textMuted : const Color(0xFF5A5A80), fontSize: 14, height: 1.7),
      bodySmall:      TextStyle(color: dark ? subtle    : const Color(0xFF6B6B9A), fontSize: 12),
    ),
  );
}
...
class AppCard extends StatelessWidget {
  ...
  final double? radius;
  final bool glowing;
  const AppCard({super.key, required this.child, this.background, this.borderColor,
    this.padding, this.onTap, this.radius, this.glowing = false});
...
class _PrimaryButtonState extends State<PrimaryButton> with SingleTickerProviderStateMixin {
  ...
  @override Widget build(BuildContext context) {
    ...
    final grad = widget.gradient ?? (widget.color != null
        ...
        builder: (_, __) => Transform.scale(scale: _scale.value,
          child: SizedBox(width: double.infinity,
            child: Container(
              ...
              child: Text(widget.label, style: TextStyle(color: Colors.white, fontSize: AppFS.btn, fontWeight: FontWeight.w800, letterSpacing: 0.3)),
            ),
          ),
        ),
      ),
...
class SecondaryButton extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
      ...
      builder: (_, __) => SizedBox(width: double.infinity,
        child: OutlinedButton(
          onPressed: onPressed,
          style: OutlinedButton.styleFrom(foregroundColor: tc.subtle,
...
class XpChip extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => AnimatedBuilder(
    ...
    builder: (_, __) => Container(
      ...
      decoration: BoxDecoration(gradient: AppTheme.certGrad, borderRadius: BorderRadius.circular(99),
...
class StepHeader extends StatelessWidget {
  ...
  const StepHeader({super.key, required this.emoji, required this.label,
    required this.title, required this.color, required this.colorLt});
  @override Widget build(BuildContext context) {
    final tc = TC.of(context);
    return AnimatedBuilder(
      animation: Listenable.merge([fontSizeNotifier, themeNotifier]),
      builder: (_, __) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(
          ...
          decoration: BoxDecoration(color: colorLt, borderRadius: BorderRadius.circular(99),
        ...
        Text(title, style: TextStyle(color: tc.textBody, fontSize: AppFS.title, fontWeight: FontWeight.w800, height: 1.3)),
      ]),
    );
  }
}
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: checked ? [BoxShadow(color: tc.green.withValues(alpha: 0.4), blurRadius: 8)] : [],
        ),
