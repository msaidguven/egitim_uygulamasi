-- Normalize Turkish slugs (including circumflex letters like â/î/û)
-- and update existing data.

CREATE EXTENSION IF NOT EXISTS unaccent;

CREATE OR REPLACE FUNCTION public.slugify_tr(p_text text)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
BEGIN
  ...
  -- Explicit Turkish/circumflex normalization first.
  v := replace(v, 'ı', 'i');
  v := replace(v, 'â', 'a');
  v := replace(v, 'î', 'i');
  v := replace(v, 'û', 'u');

  -- General latin diacritics normalization.
  v := unaccent(v);

  -- Replace non [a-z0-9] with hyphen and collapse repeats.
...
SET slug = public.slugify_tr(name)
...
-- Future inserts for topics in this RPC should use the same slug normalization.
