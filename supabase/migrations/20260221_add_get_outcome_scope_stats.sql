-- Compute solved/correct/wrong stats for an outcome-filtered weekly scope.
-- This centralizes UI logic into DB to avoid client-side drift.

CREATE OR REPLACE FUNCTION public.get_outcome_scope_stats(
  p_user_id uuid,
  p_unit_id bigint,
  p_topic_id bigint,
...
BEGIN
  ...
  WITH candidate_sessions AS (
    ...
    WHERE ts.user_id = p_user_id
      ...
      AND ts.settings->>'type' IN ('weekly', 'weekly_outcome')
  ),
  matched_sessions AS (
    SELECT cs.id
    FROM candidate_sessions cs
    WHERE cs.settings->>'type' = 'weekly'
       OR (
         cs.settings->>'type' = 'weekly_outcome'
         AND (
...
-- Compatibility overload for clients sending integer[] arrays.
CREATE OR REPLACE FUNCTION public.get_outcome_scope_stats(
  p_user_id uuid,
  p_unit_id bigint,
  p_topic_id bigint,
  p_curriculum_week integer,
  p_outcome_ids integer[],
...
STABLE
AS $$
  SELECT public.get_outcome_scope_stats(
    p_user_id,
    p_unit_id,
    p_topic_id,
    p_curriculum_week,
    ARRAY(SELECT x::bigint FROM unnest(p_outcome_ids) AS x),
