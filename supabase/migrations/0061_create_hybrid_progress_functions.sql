-- Migration: 0061_create_hybrid_progress_functions.sql
-- Bu migration, hibrit tamamlama modeli için gerekli olan RPC ve Trigger fonksiyonlarını oluşturur.

-- 1. Adım: Kullanıcının "bitirme niyetini" kaydeden RPC.
...
AS $$
BEGIN
    -- İlgili hafta için bir kayıt oluştur veya mevcutsa `declared_completed_at` alanını güncelle.
...
-- 2. Adım: Sistemin otomatik değerlendirmesini yapan TRIGGER fonksiyonu.
...
BEGIN
    -- Cevaplanan sorunun hangi derse, sınıfa ve haftaya ait olduğunu bul.
    ...
    -- Eğer soru bir haftaya atanmamışsa, işlemi sonlandır.
    IF v_week_no IS NULL THEN
        RETURN NEW;
    END IF;

    -- O haftadaki toplam soru sayısını bul.
    SELECT count(*)
    ...
    IF v_total_questions_in_week = 0 THEN
        v_total_questions_in_week := 1; -- Bölme hatasını önle
    END IF;

    -- Kullanıcının o hafta çözdüğü benzersiz soru sayısını ve doğru sayısını bul.
    SELECT
    ...
    -- `user_progress` tablosuna kaydı ekle veya güncelle.
    ...
    -- Koşullar sağlanıyorsa, haftayı "sistem tarafından tamamlandı" olarak işaretle.
    -- Koşul: En az %70'i çözülmüş VE başarı oranı en az %85 olmalı.
...
-- 3. Adım: Trigger'ı oluştur.
-- Bu trigger, `test_session_answers` tablosuna her yeni kayıt eklendiğinde çalışır.
DROP TRIGGER IF EXISTS on_new_answer_update_progress ON public.test_session_answers;
CREATE TRIGGER on_new_answer_update_progress
  AFTER INSERT ON public.test_session_answers
  FOR EACH ROW
  WHEN (NEW.user_id IS NOT NULL)
  EXECUTE FUNCTION public.sync_user_progress_on_answer();
