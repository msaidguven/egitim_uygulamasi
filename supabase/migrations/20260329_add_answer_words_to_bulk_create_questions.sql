-- Persist answer_words for classical questions coming from Lesson V11 parsers.
CREATE OR REPLACE FUNCTION public.bulk_create_questions(
    ...
    p_outcome_ids BIGINT[] DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
    ...
    v_inserted_question_ids JSONB := '[]'::JSONB;
BEGIN
    ...
    LOOP
        BEGIN
            ...
            IF p_outcome_ids IS NOT NULL AND array_length(p_outcome_ids, 1) > 0 THEN
                INSERT INTO public.question_outcomes (question_id, outcome_id)
                SELECT new_question_id, o.id
                FROM public.outcomes o
                WHERE o.id = ANY(p_outcome_ids)
                  AND o.topic_id = p_topic_id
            ...
            ELSIF question_type_text = 'classical' THEN
                IF question_data ? 'model_answer' THEN
                    INSERT INTO public.question_classical (question_id, model_answer, answer_words)
                    VALUES (
                        new_question_id,
                        question_data->>'model_answer',
                        CASE
                            WHEN jsonb_typeof(question_data->'answer_words') = 'array' THEN
                                ARRAY(SELECT jsonb_array_elements_text(question_data->'answer_words'))
                            ELSE
                                ARRAY[]::TEXT[]
                        END
                    );
            ...
            v_inserted_question_ids := v_inserted_question_ids || to_jsonb(new_question_id);
    ...
    RETURN jsonb_build_object(
        ...
        'inserted_question_ids', v_inserted_question_ids,
        'errors', v_errors
    );
END;
$$;
